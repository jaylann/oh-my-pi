...
## Inline agent selector and click to focus

While subagents run, a `Subagents` selector below the editor lists every live agent — sync task calls and detached background spawns alike. From an empty editor, press `↓` to enter it, use `↑`/`↓` or `j`/`k` to select an agent, press `Enter` to open that agent's live session, or press `Esc` to return to the editor.

The selector stays short: it shows a few rows plus an expander (`display.pinnedAgents: collapsed`, the default), lists everything (`full`), or hides entirely (`off`). Clicking the expander toggles between the two while `tui.mouse` is on.

Enable `tui.mouse` to click live subagent cards and selector rows directly in the main session, without opening the Hub first. A click focuses that card's most recent agent (a selector row focuses its exact agent); focusing a parked agent revives it. Hovering a live target lights it up first, so you can see what a click will open.
