...
- optional `tools`, `spawns`, prioritized `model` list, `thinkingLevel`, `output`, `blocking`, `autoloadSkills`, `mcpServers`, `readSummarize`, `prewalk`, `advisor`
...
- `mcpServers` accepts CSV or an array and explicitly authorizes those configured MCP servers for the child. Declared on-demand servers connect during spawn; only the child's registry, prompt routes, catalogs, and server instructions receive them. Unknown, disabled, or filtered names fail the spawn. Agents without this field retain the backward-compatible startup MCP set.
