...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, agentSpec?, task, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.enabled` is true **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off).

- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background rendered into every spawned subagent's system prompt (`CONTEXT` section); `agent`, `agentSpec`, `outputSchema`, and `schemaMode` are per item. `effort` is added only when its setting enables it; `isolated` additionally requires plan mode to be disabled.
...
| `agent` | `string` | No | Named agent type whose prompt and ordinary policy form the base for this item (e.g. `scout`). Defaults to the spawn policy's default agent (usually `task`); items in one batch call may use different agent types. Item field in batch shape, top-level in flat shape. |
| `agentSpec` | `object` | No | Ephemeral overrides for this spawn: `model` (`string \| string[]`), `thinkingLevel`, tool allowlist, `spawns` (`string[] \| "*"`), and `autoloadSkills`. Omitted `spawns` denies nested delegation. Every reference is validated before launch; the specification is never added to agent discovery or a global registry. |
...
6. It resolves each spawn's requested named `agent`, rejects unknown or settings-disabled agents, and enforces parent spawn policy plus `PI_BLOCKED_AGENT` self-recursion prevention. When present, `agentSpec` is validated and layered onto that definition only for this launch; no prompt replacement is accepted and nested spawning defaults to denied.
7. Model priority: inline `agentSpec.model` → `task.agentModelOverrides` → agent frontmatter → configured task role/session fallback. Output schema priority: per-call `outputSchema` → agent frontmatter `output` → inherited parent session schema.
...
13. Child tool availability: explicit `agent.tools` if provided; inline `agentSpec.tools` additionally prevents unlisted extension and MCP tools from becoming active while retaining normal discovery and proxy wiring; auto-add `task` when the agent has `spawns` and depth allows; strip `task` at `task.maxRecursionDepth`; ensure `hub` is present in explicit tool lists; expand `exec` to `eval` + `bash`; strip parent-owned `todo` — unless the spawn is prewalk-armed, whose plan nudge + todo gate need the child to commit its own todo list before the model hand-off.
